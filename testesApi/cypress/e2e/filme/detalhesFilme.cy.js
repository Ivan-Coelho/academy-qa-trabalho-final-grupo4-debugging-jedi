      ...
      cy.criarUsuarioCritico().then(function (dadosCritico1) {
        ...
        cy.criarReview(idFilme, userCritico1.token).then(function (response) {
          ...
          cy.criarUsuarioCritico().then(function (dadosCritico2) {
            ...
            cy.criarReview(idFilme, userCritico2.token).then(function (
              response
            ) {
              ...
              cy.request({
                ...
                expect(response.body.reviews[0].user.id).to.equal(
                  userCritico1.id
                );
                expect(response.body.reviews[0].user.name).to.equal(
                  userCritico1.nome
                ...
                expect(response.body.reviews[1].reviewText).to.equal(
                  comentario2
                );
                ...
                expect(response.body.reviews[1].user.id).to.equal(
                  userCritico2.id
                );
                expect(response.body.reviews[1].user.name).to.equal(
                  userCritico2.nome
                );
