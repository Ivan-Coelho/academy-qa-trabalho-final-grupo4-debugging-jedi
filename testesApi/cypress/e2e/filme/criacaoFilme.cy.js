  ...
  describe("Cadastro de filme como usuario adminstrador", function () {
        ...
        cy.request({
          ...
          cy.request({
            ...
            expect(response.body).to.have.property(
              "description",
              dadosFilme.description
            );
            expect(response.body).to.have.property(
              "durationInMinutes",
              dadosFilme.durationInMinutes
            );
            expect(response.body).to.have.property(
              "releaseYear",
              dadosFilme.releaseYear
            );
          });
        });
      });
    ...
    it("Usuário administrador tenta cadastrar um filme com quantidade de caracteres do titulo invalidos", function () {
      cy.request({
        ...
        body: {
          title:
            "A Incrível Jornada da Caneta Azul e Seus Amigos na Terra dos Sonhos Encantados em Uma Aventura Sem Fim",
          genre: "Musical",
          description:
            "Caneta azul, azul caneta, Caneta azul tá marcada com minhas letra",
    ...
    it("Cadastrar o mesmo filme com novas informações", function () {
      ...
      const novasInformacoesFilme = {
        ...
        description:
          "Regravação de As forças imperais comandadas por Darth Vader lançam um ataque contra os membros da resistência, que são obrigados a fugir. Enquanto isso, Luke Skywalker tenta encontrar o Mestre Yoda, que poderá ensiná-lo a dominar a Força e torná-lo um cavaleiro Jedi.",
        durationInMinutes: 124,
        releaseYear: 2000,
      };

      cy.cadastrarFilme(dadosAdmin.token, novasInformacoesFilme).then(function (
        filme2
      ) {
